...
Lambda manages the function's asynchronous invocation queue and attempts to retry failed events automatically\. If the function returns an error, Lambda attempts to run it two more times, with a one\-minute wait between the first two attempts, and two minutes between the second and third attempts\. Function errors include errors returned by the function's code and errors returned by the function's runtime, such as timeouts\. If all three attempts fail, Lambda sends the event to a [dead\-letter queue](#dlq), if it's configured\.
