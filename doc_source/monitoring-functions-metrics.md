...
| DeadLetterErrors |  The Dead Letter Errors metric will be incremented when Lambda is unable to write the failed event payload to your configured [Dead Letter Queues](dlq.md)\. This could be due to the following: [\[See the AWS documentation website for more details\]](http://docs.aws.amazon.com/lambda/latest/dg/monitoring-functions-metrics.html) Units: Count  | 
...
| UnreservedConcurrentExecutions |  Emitted as an aggregate metric for all functions in the account only\. Not applicable for functions, versions, or aliases\. Represents the sum of the concurrency of the functions that don't have a custom concurrency limit specified\. Must be viewed as an average metric if aggregated across a time period\.  Units: Count | 
...
| Resource |  Filters the data you request for a Lambda function resource\.  | 
| Version |  Filters the data you request for a Lambda version\.  | 
| Alias |  Filters the data you request for a Lambda alias\.  | 
| Executed Version |  Filters the metric data by the executed Lambda function version\. This only applies to alias invocations\.  | 