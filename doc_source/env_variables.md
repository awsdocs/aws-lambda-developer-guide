...
If you’re using the default KMS service key for Lambda, then no additional IAM permissions are required in your function execution role – your role will just work automatically without changes\. If you’re supplying your own \(custom\) KMS key, the user that manages the Lambda function must have permission to use the KMS key \(`kms:Decrypt` and `kms:CreateGrant`\)\\\. For more information on KMS keys, see the [Using Key Policies in AWS KMS](https://docs.aws.amazon.com/kms/latest/developerguide/key-policies.html)\.
